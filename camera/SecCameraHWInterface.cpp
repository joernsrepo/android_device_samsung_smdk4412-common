...
void CameraHardwareSec::initDefaultParameters(int cameraId)
{
    ...
    if (!mUseInternalISP) {
        ...
        p.set(CameraParameters::KEY_ZOOM_RATIOS, "31");
    }
...
int CameraHardwareSec::pictureThread()
{
    ...
    if (mMsgEnabled & CAMERA_MSG_COMPRESSED_IMAGE) {
        ...
        memcpy(ImageStart, ((unsigned char*)JpegHeap->data + 2), JpegImageSize - 2);
...
static struct camera_info sCameraInfo[] = {
    {
        CAMERA_FACING_BACK,
        90,  /* orientation */
        1, /* device version */
        NULL, /* static camera characteristics */
    },
    {
        CAMERA_FACING_FRONT,
        90,  /* orientation */
...
{
    ALOGV("%s", __func__);
    memcpy(cameraInfo, &sCameraInfo[cameraId], sizeof(struct camera_info));
    return 0;
}
